...
FROM BOOK B                                     -- 3개의 테이블 도서 정보(BOOK), 저자정보(AUTHOR), 판매정보(BOOK_SALES) 중에서 도서정보의 데이블이 공통된 컬럼을 가지고 있기 때문에 중심으로 잡았습니다.
...
## 배운점
- 3개의 테이블을 공통 컬럼 기준으로 조인하여 데이터를 통합하고, GROUP BY와 필터링을 통하여 쿼리를 작성하는법을 배웠습니다.
